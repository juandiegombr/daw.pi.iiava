...
.PHONY: dev-build dev-up dev-upf dev-down dev-rebuild logs-dev-backend seed seed-user
...
seed-user: ## Create a custom user interactively
	$(COMPOSE) -f docker-compose.dev.yml exec -it backend npm run seed:user
